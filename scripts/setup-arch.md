...
| Step | Change | Reason |
| --- | --- | --- |
| Packages | Required: `base-devel git curl pciutils usbutils clang lld llvm tcc cpio bc flex bison libelf openssl perl cmake ninja`, plus `rust` (default) or `rustup` (`PHI_RUSTUP=1`). Optional, installed one by one and only warned about on failure: `poppler musl kernel-headers-musl` | Host workspace build, kernel build prerequisites; the optional ones are for the card toolchain (P2) and PDF text extraction |
