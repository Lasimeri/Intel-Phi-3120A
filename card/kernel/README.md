...
| 0025 | `x86/knc: /dev/phiblk0, a block device served by the host over ring channel 4` | `kernel/knc_blk.c`, `asm/knc_ring.h` | persistent storage: blk-mq over ring kind 4; direct mode hands the request pages to the host (DMA engine, coherent) or bounce mode copies through 512 KiB uncached slots (aperture writes are not seen by the caches; measured 2026-09-16), chosen by the identify answer; disk registered from a workqueue |
