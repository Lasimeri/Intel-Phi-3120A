...
| **P5 SSH** | dropbear on the card, key provisioning by `phictl`, host `~/.ssh/config` entry | `ssh phi uname -a` prints the mainline version string | In progress 2026-09-14: `phi0` over the ring (kernel patch 0021 + `phictl boot --net`), dropbear 2025.88 static in the initramfs with host keys and the user's public key (post-quantum KEMs off: inline-asm `cmov`); `phi0` verified (ping 2.6 ms), dropbear runs from init and answers on 22 (test 13), interactive login by the user pending. The non-SSH direct-access tool WORKS from an unprivileged shell (`phictl boot --serve` + `exec/put/get/status`; ring channel 3, `/dev/phirpc`, Rust `phi-agent`; ADR 0008, `docs/howto/direct-access.md`, `docs/results/2026-09-14-p5-net-rpc.md`) |
...
| **P10 Stretch** | DMA engine for bulk transfer, vector ISA access via inline assembly, thermal and power readout, `perf` via the mainline KNC PMU driver | Each recorded with a measurement | Vector ISA: done 2026-09-15 (`host/crates/knc-mvex` encoder, patch 0024 vector state, Mandelbrot on the VPU 1.47x the host AVX2: `docs/results/2026-09-15-vpu.md`); DMA engine: done 2026-09-16 (host-owned channel, status-descriptor completion, the disk at 846 MB/s: `docs/results/2026-09-16-dma.md`); persistent storage done (`2026-09-16-storage.md`); host memory as swap and shared window done (patch 0026); thermal readout done (hwmon, patch 0027) and perf counters done (`phiperf`, `2026-09-16-sensors.md`); remote resource viewer done 2026-09-17 (`phitop`, ADR 0009, `2026-09-17-phitop.md`); power readout open (SMC over I2C) |
