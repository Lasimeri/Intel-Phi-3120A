...
| **P7 C on the card** | Native clang, musl headers, `make`, `binutils`; pthread test that uses all threads | A pthread program compiled *on the card* runs on 228 threads and passes `phi-isa-audit` | In progress 2026-09-14: card libc++ built (`toolchain/libcxx/build.sh`), clang+lld cross-built for the card (`PHI_LLVM_VARIANT=card`), packaged as `/opt/phi` (`card/userland/components/clang.sh`), loaded by `clang-push.sh` through the direct-access tool |
