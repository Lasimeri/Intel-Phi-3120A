...
| SSH through the forwarder | holders of an authorized private key | dropbear on the card, public keys only (`-s`), root only, bound to the card's own address (`10.9.0.2:22`) rather than every address, and not started at all when there is no `phi0`; the forwarder listens on 127.0.0.1:2222 and nothing else (`host/crates/phictl/src/forward.md`), so the host's loopback is the only way in |
...
image file is mode 0600 since 2026-09-16. Since 2026-09-19 the card's
`/etc` is persistent, so both are copied out of the image over the
persistent copies on **every** boot: a rebuilt image always decides who may
log in, and an edit made on the card cannot outlive it
(`card/initramfs/build.md`). `~/.ssh/phi_ed25519` has no
